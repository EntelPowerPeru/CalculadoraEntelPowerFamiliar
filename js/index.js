    ...
    document
      ...
      .forEach((elemento) => {
        elemento.addEventListener("change", (evento) => {
          ...
          const pago_sin_descuento = !numero_lineas || !costo_plan_familiar ? "" : Number(lineas_nuevas_actuales * costo_plan_familiar);

          const pago_con_descuento = !numero_lineas || !costo_plan_familiar ? "" : Number((lineas_nuevas_actuales - 1) * promocion + costo_plan_familiar);

          const pago_ahorro_mensual_familiar = !pago_sin_descuento || !pago_con_descuento ? "" : Number(pago_sin_descuento - pago_con_descuento);
