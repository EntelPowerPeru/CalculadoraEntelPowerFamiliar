...
const HELPERS = {
  ...
  establecer_datos() {
    ...
    /*const plan_sugerido_linea_adicional_2_v2 = HELPERS.plan_sugerido_linea_adicional_v2(
      pago_total_plan_actual,
      [plan_linea_1, plan_linea_2, plan_linea_3, plan_linea_4, plan_linea_5],
      numero_lineas,
      lineas_nuevas_actuales
    );*/
    const plan_sugerido_linea_adicional_2_v2 = HELPERS.plan_sugerido_linea_adicional_2_v3(
      pago_total_plan_actual,
      lineas_nuevas_actuales,
      [plan_linea_1, plan_linea_2, plan_linea_3, plan_linea_4, plan_linea_5],
      numero_lineas
    ...
    const plan_sugerido_linea_adicional_v2 = HELPERS.plan_sugerido_linea_adicional_v3(
      cbo_plan_linea_1_texto,
      cbo_plan_linea_2_texto,
    ...
    const pago_sin_descuento = HELPERS.pago_mensual_sin_beneficio_llaa(cbo_plan_familiar_texto, lineas_nuevas_actuales);

    //const pago_con_descuento = !numero_lineas || !costo_plan_familiar ? "" : Number((lineas_nuevas_actuales - 1) * promocion + costo_plan_familiar);
    //const pago_con_descuento = F66_J81.find(e => e.plan === cbo_plan_familiar_texto && e.lineas === lineas_nuevas_actuales)?.monto
    const pago_con_descuento = HELPERS.pago_mensual_con_beneficio_llaa(cbo_plan_familiar_texto, lineas_nuevas_actuales);
    ...
    document.querySelector("#txt_pago_sin_descuento").value = Number(pago_sin_descuento) ? pago_sin_descuento.toFixed(2) : pago_sin_descuento;
    document.querySelector("#txt_pago_con_descuento").value = Number(pago_con_descuento) ? pago_con_descuento.toFixed(2) : pago_con_descuento;
    ...
    try {
      if (lineas_adicionales === 0) {
        ...
        if (lista_montos_planes_actuales.max() < 44.9) {
          ...
          let indice = AG32_AH46.filter(e => e.monto <= lista_montos_planes_actuales.max()).length - 1;
    ...
    try {
      if (lineas_adicionales === 0) {
        ...
        if (lista_montos_planes_actuales.max() < 44.9) {
          ...
          let lista_aux = [...AK_AL];
          //lista_aux.pop();
          let indice = lista_aux.filter(e => e.monto <= lista_montos_planes_actuales.max()).length;

          return lista_aux[indice].plan;
        }
      }
    } catch (error) {
      return "-";
  ...
  t26_v3(lista_montos_planes_actuales, lineas_adicionales) {
    const al32 = "ENTEL POWER FAMILIAR 39.90 C";
    ...
    try {
      if (lineas_adicionales === 0) {
        ...
        if (lista_montos_planes_actuales.max() < 44.9) {
          ...
          let indice = AK32_AL46.filter(e => e.monto <= lista_montos_planes_actuales.max()).length - 1;

          return AK32_AL46[indice].plan;
        }
      }
    } catch (error) {
      return "-";
  ...
  v26_v3(total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    const t26 = this.t26_v3(lista_montos_planes_actuales, lineas_adicionales).toLowerCase();

    return F66_J80.find(e => e.plan.toLowerCase() === t26 && e.lineas === total_lineas).monto;
  },
  t27_v3(total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    let k45_n45 = [];
    const al46 = "ENTEL POWER FAMILIAR 199.90";

    try {
      const v26 = this.v26_v3(total_lineas, lista_montos_planes_actuales, lineas_adicionales);
      const t26 = this.t26_v3(lista_montos_planes_actuales, lineas_adicionales);

      for (let i = 0; i < 4; i++) {
        let lineas = i + 2;
        let lista_aux = F66_J80.filter(e => e.lineas === lineas);
        let indice = lista_aux.filter(e => e.monto <= total_planes_actuales).length;
        const plan = lista_aux[indice].plan;

        if (v26 < total_planes_actuales) {
          k45_n45.push({ lineas, plan });
        } else {
          k45_n45.push({ lineas, plan: t26 });
        }
  ...
  plan_sugerido_linea_adicional_v3(plan_actual_1, plan_actual_2, psla2, total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    if (plan_actual_1 !== plan_actual_2) {
      return psla2;
    }
    ...
    try {
      ...
      return this.t27_v3(total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales);
    } catch (error) {
      return "ERROR";
    }
  },
  ...
  t45_v3(lista_montos_planes_actuales, lineas_adicionales) {
    const ap48 = "POWER 49.90";

    try {
      if (lineas_adicionales === 0) {
        return "-";
      } else {
        if (lista_montos_planes_actuales.max() < 49.9) {
          return ap48;
        } else {
          let indice = AO48_AP54.filter(e => e.monto <= lista_montos_planes_actuales.max()).length;

          return AO48_AP54[indice].plan;
        }
      }
    } catch (error) {
      return "-";
  ...
  v45_v3(total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    const t45 = this.t45_v3(lista_montos_planes_actuales, lineas_adicionales).toLowerCase();

    return F55_J80.find(e => e.plan.toLowerCase() === t45 && e.lineas === total_lineas).monto;
  },
  t46_v3(total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    debugger
    let k50_n50 = [];
    const ap42 = "ENTEL POWER 129.90 SD C";

    try {
      const v45 = this.v45_v3(total_lineas, lista_montos_planes_actuales, lineas_adicionales);
      const t45 = this.t45_v3(lista_montos_planes_actuales, lineas_adicionales);

      for (let i = 0; i < 4; i++) {
        let lineas = i + 2;
        let lista_aux = F55_J60.filter(e => e.lineas === lineas);
        let indice = lista_aux.filter(e => e.monto <= total_planes_actuales).length;
        const plan = lista_aux.length >= indice ? "" : lista_aux[indice].plan;

        if (v45 < total_planes_actuales) {
          k50_n50.push({ lineas, plan });
        } else {
          k50_n50.push({ lineas, plan: t45 });
        }
  ...
  plan_sugerido_linea_adicional_2_v3(total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales) {
    try {
      if (lineas_adicionales < 1) {
        return "NO APLICA";
      }

      return this.t46_v3(total_planes_actuales, total_lineas, lista_montos_planes_actuales, lineas_adicionales);
    } catch (error) {
      return "ERROR";
    }
  },
  ...
  p18_v3(plan_sugerido) {
    try {
      return AK_AL.find(e => e.plan.toLowerCase() === plan_sugerido.toLowerCase()).monto;
    } catch (error) {
      return "0";
    }
  },
  pago_mensual_sin_beneficio_llaa(plan_sugerido, total_lineas) {
    const p18 = this.p18_v3(plan_sugerido);
    ...
    if (p18 == 0) {
      // La celda p19 está vacia, se devuelve 0
      return 0;
    } else {
      return p18 * total_lineas;
    }
  },
  pago_mensual_con_beneficio_llaa(plan_sugerido, total_lineas) {
    try {
      const valor = F55_J80.find(x => x.plan.toLowerCase() === plan_sugerido.toLowerCase() && x.lineas === total_lineas).monto;
