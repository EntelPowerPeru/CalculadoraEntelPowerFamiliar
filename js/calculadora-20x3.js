...
const HELPERS = {
  ...
  t24(lineas_adicionales, lista_montos_planes_actuales) {
    ...
    const objeto = LISTA_PLANES_FAMILIARES.reduce((prev, curr) => Math.abs(curr.monto - lista_montos_planes_actuales.max()) < Math.abs(prev.monto - lista_montos_planes_actuales.max()) ? curr : prev);
    let indice_plan_familiar_mas_uno = LISTA_PLANES_FAMILIARES.findIndex(e => { e.plan === objeto.plan }) + 1;

    /*for (let i = 0; i < LISTA_PLANES_FAMILIARES.length; i++) {
      if (lista_montos_planes_actuales.max() === LISTA_PLANES_FAMILIARES[i].monto) {
        indice_plan_familiar_mas_uno = i + 1;
        break;
      }
    }*/

    if (!indice_plan_familiar_mas_uno < 1) {
      return "-";
    }

    return LISTA_PLANES_FAMILIARES[indice_plan_familiar_mas_uno].plan;
  },
  ...
  t36(q_lineas, total_planes_actuales) {
    //const plan = LISTA_MENSUAL_CON_FAMILIA.find(e => e.monto === total_planes_actuales && e.lineas === q_lineas)?.plan;
    //const plan = LISTA_MENSUAL_CON_FAMILIA.find(e.lineas === q_lineas)?.plan;
    ...
    const objeto = lista.reduce((prev, curr) => Math.abs(curr.monto - total_planes_actuales) < Math.abs(prev.monto - total_planes_actuales) ? curr : prev);
  ...
  t25(lineas_adicionales, total_lineas, lista_montos_planes_actuales, total_planes_actuales) {
    ...
    let indice_plan_familiar_mas_uno;

    for (let i = 0; i < LISTA_MENSUAL_CON_FAMILIA.length; i++) {
      if (total_planes_actuales === LISTA_MENSUAL_CON_FAMILIA[i].monto && total_lineas === LISTA_MENSUAL_CON_FAMILIA[i].lineas) {
        let indice_plan_familiar_mas_uno = i + 1;

        break;
      }
    }

    if (!indice_plan_familiar_mas_uno) {
      return "ENTEL POWER FAMILIAR 129.90 SD N"
    }

    return LISTA_MENSUAL_CON_FAMILIA[indice_plan_familiar_mas_uno].plan;
  },
