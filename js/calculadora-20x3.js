    ...
    document
      ...
      .forEach((elemento) => {
        elemento.addEventListener("change", (evento) => {
          ...
          const pago_sin_descuento = !lineas_nuevas_actuales || !costo_plan_familiar ? "" : Number(lineas_nuevas_actuales * costo_plan_familiar);

          const pago_con_descuento = !lineas_nuevas_actuales || !costo_plan_familiar ? "" : Number((lineas_nuevas_actuales - 1) * promocion + costo_plan_familiar);
          ...
          const plan_familiar_texto = cbo_plan_familiar.options[cbo_plan_familiar.selectedIndex].text;

          const pago_con_20x3 = numero_lineas && plan_familiar_texto !== "-" && lineas_nuevas_actuales !== "0" ?
            (!q_lineas_actual ?
              LISTA_20x3.find(e => e.lineas_nuevas_actuales === lineas_nuevas_actuales && e.text === plan_familiar_texto).id
              :
              LISTA_LINEAS_20x3.find(e => e.q_lineas_actuales === q_lineas_actual && e.lineas_nuevas_actuales === lineas_nuevas_actuales && e.text === plan_familiar_texto).id)
            :
            0;

          const pago_ahorro_mensual_familiar = !pago_sin_descuento || !pago_con_20x3 ? "" : Number(pago_sin_descuento - pago_con_20x3);

          const pago_ahorro_anual_familiar = (pago_sin_descuento * 12) - ((pago_con_descuento * 9) + (pago_con_20x3 * 3));

          const pago_diferencia_factura = !pago_total_plan_actual ? "" : Number(pago_con_20x3 - pago_total_plan_actual);
          ...
          document.querySelector("#txt_tipo_cliente").value = q_lineas_actual ? "Base" : "Nuevo";
...
const INICIALIZAR_ELEMENTOS = {
  ...
  inputs() {
    ...
    document.querySelector("#txt_tipo_cliente").value = Number(document.querySelector("#txt_q_lineas_actual").value) ? "Base" : "Nuevo";
  },
};
